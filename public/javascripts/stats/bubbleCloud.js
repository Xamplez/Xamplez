app.factory("BubbleCloud", [function () {
	...
	root.plotData = function(d3selection, data) {
	  ...
	  d3selection.datum(data).call(plot);
	};

	return {
		display: function (data, selector) {
			selector = selector || "#bubbleCloud";
			var d3selection = d3.select(selector);
			...
			$(window).on("resize", function () {
				...
				if (newWidth != width) {
					...
					root.plotData(d3selection, data);
				}
			});
